(function($) {
    ...
    function updateTwoFactorCode() {
        ...
        $('.code-qr-content input', this._container).each(function( index, value ) {
            if(!$(value).val()) {
                isComplete = false;
        ...
        if(isComplete) {
            Livewire.emit('checkTwoFactorCode', code);
            resetTwoFactorCode();
        }
    }

    function resetTwoFactorCode() {
        $('.code-qr-content input')[0].select();
        $('.code-qr-content input')[0].focus();
